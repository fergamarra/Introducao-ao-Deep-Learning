...
for opt_name in optimizers:
    ...
    model.compile(loss='categorical_crossentropy', optimizer=eval(opt_name), metrics=['accuracy'])
...
historydf.xs('loss', axis=1, level='metric').plot(ylim=(0,2), style = ["k--","k-"])
plt.xlabel("Epocas")
plt.title("Loss")
plt.show()


historydf.xs('accuracy', axis=1, level='metric').plot(ylim=(0,1), style = ["k--","k-"])
plt.title("Acuracia")
plt.xlabel("Epocas")
